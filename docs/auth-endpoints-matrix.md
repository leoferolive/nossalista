...
| `POST` | `/mcp` | clientes MCP externos (Claude Code/Desktop, Cursor) | Requer JWT ou PAT (`nlmcp_...`). Enforcement de escopo READ e por tool, nao por metodo HTTP — ver `docs/mcp.md` e `docs/DECISIONS.md` (D-020) |
...
- **Servidor MCP (`/mcp`):** aceita os mesmos tokens (JWT ou PAT) que `/api/**`, mas a regra de escopo READ/READ_WRITE e aplicada dentro de cada tool (`McpSecurityContext`), nao pelo metodo HTTP — todo o protocolo MCP trafega via `POST`. Ver `docs/mcp.md` e `docs/DECISIONS.md` (D-020).
