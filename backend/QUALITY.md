...
- **Baseline de versoes (upgrade patch/minor 2026-07-27)**: Spring Boot `4.1.0` (BOM sobe spring-framework `7.0.8`, spring-security `7.1.0`, tomcat `11.0.22`, flyway `12.4.0`); overrides via `<properties>` mantidos: tomcat `11.0.23`, netty `4.2.15.Final`, postgresql `42.7.13`; swagger-ui `5.32.11` (DOMPurify `3.4.0`); jjwt `0.13.0`. `web-push` permanece em `5.1.1` — a `5.1.2` reescopou `httpasyncclient`/`httpclient` de `compile` para `runtime` no proprio POM, mas `PushService.send()` retorna `org.apache.http.HttpResponse` na API publica, quebrando a compilacao.
