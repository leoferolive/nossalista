...
export const useLists = (): UseListsReturn => {
   ...
   * Cria uma nova lista
   */
  const createList = useCallback(async (request: CreateListRequest): Promise<ListResponse> => {
    setLoading(true)
    setError(null)
   ...
   */
  const updateListName = useCallback(async (id: string, name: string): Promise<ListResponse> => {
