...
describe('ListView - Delete Functionality', () => {
  ...
  it('deve manter modal aberto em erro 500 para retry', async () => {
    ...
    await waitFor(() => {
      expect(mockShowToast).toHaveBeenCalledWith('Erro ao excluir lista. Tente novamente.', 'error')
    })
...
describe('ListView - WebSocket Integration', () => {
  ...
  const createWsMessage = ({
    ...
    channel:
      channel ??
      (type === 'PRESENCE_SNAPSHOT' || type.startsWith('MEMBER_') ? 'presence' : 'items'),
    revision:
      channel === 'presence' || type === 'PRESENCE_SNAPSHOT' || type.startsWith('MEMBER_')
        ? undefined
        : Date.now(),
    type,
    payload,
  ...
  it('deve adicionar item ao estado quando recebe ITEM_ADDED de outro usuário', () => {
    ...
    mockSubscribe.mockImplementation(
      (_listId: string, _channel: string, handler: (msg: unknown) => void) => {
  ...
  it('NÃO deve exibir Toast quando ITEM_ADDED é do próprio usuário', () => {
    ...
    expect(mockShowToast).not.toHaveBeenCalledWith(expect.stringContaining('adicionou'), 'info')
  ...
  it('NÃO deve exibir Toast quando ITEM_UPDATED é do próprio usuário', () => {
    ...
    expect(mockSetItems).not.toHaveBeenCalled()
    expect(mockShowToast).not.toHaveBeenCalledWith(expect.stringContaining('editou'), 'info')
  })
